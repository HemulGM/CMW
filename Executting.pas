...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, StructUnit, Vcl.ExtCtrls, Vcl.ComCtrls, COCUtils, Vcl.ImgList, OSInfo;
...
implementation
 uses System.IniFiles, Winapi.ShellAPI;
...
begin
 ...
 case res of
  SE_ERR_FNF:         begin TryMore:=True; Log(['Попытка №1 - файл не найден.']); end;        { file not found }
  SE_ERR_PNF:         begin TryMore:=True; Log(['Попытка №1 - Путь не найден.']); end;        { path not found }
  SE_ERR_ACCESSDENIED:begin TryMore:=True; Log(['Попытка №1 - Нет доступа.']); end;           { access denied  }
  SE_ERR_OOM:         begin TryMore:=True; Log(['Попытка №1 - Не хватает памяти.']); end;     { out of memory  }
  SE_ERR_DLLNOTFOUND: begin TryMore:=True; Log(['Попытка №1 - Библиотека не доступна.']); end;{ dll not found  }
  0:                  begin TryMore:=True; Log(['Попытка №1 - Команда не выполнена.']); end;
 else begin TryMore:=False; Log(['Попытка №1 - Выполнена команда:', Command+' '+Parameters, res]); end;
 end;
 if not TryMore then Exit(True);
 res:=ShellExecute(0, 'open', PWideChar(Command), PWideChar(Parameters), '', SW_NORMAL);
 case res of
  SE_ERR_FNF:         begin TryMore:=True; Log(['Попытка №2 - файл не найден.']); end;        { file not found }
  SE_ERR_PNF:         begin TryMore:=True; Log(['Попытка №2 - Путь не найден.']); end;        { path not found }
  SE_ERR_ACCESSDENIED:begin TryMore:=True; Log(['Попытка №2 - Нет доступа.']); end;           { access denied  }
  SE_ERR_OOM:         begin TryMore:=True; Log(['Попытка №2 - Не хватает памяти.']); end;     { out of memory  }
  SE_ERR_DLLNOTFOUND: begin TryMore:=True; Log(['Попытка №2 - Библиотека не доступна.']); end;{ dll not found  }
  0:                  begin TryMore:=True; Log(['Попытка №2 - Команда не выполнена.']); end;
  SE_ERR_SHARE:       begin TryMore:=True; Log(['Попытка №2 - SE_ERR_SHARE.']); end;
  SE_ERR_ASSOCINCOMPLETE:begin TryMore:=True; Log(['Попытка №2 - SE_ERR_ASSOCINCOMPLETE.']); end;
  SE_ERR_DDETIMEOUT:  begin TryMore:=True; Log(['Попытка №2 - SE_ERR_DDETIMEOUT.']); end;
  SE_ERR_DDEFAIL:     begin TryMore:=True; Log(['Попытка №2 - SE_ERR_DDEFAIL.']); end;
  SE_ERR_DDEBUSY:     begin TryMore:=True; Log(['Попытка №2 - SE_ERR_DDEBUSY.']); end;
  SE_ERR_NOASSOC:     begin TryMore:=True; Log(['Попытка №2 - SE_ERR_NOASSOC.']); end;
 else begin TryMore:=False; Log(['Попытка №2 - Выполнена команда:', Command+' '+Parameters, res]); end;
 end;
 if TryMore then
  begin
...
begin
 ...
 if not Assigned(FDisableIcon) then FI:=-1
 else FI:=ListView.SmallImages.AddIcon(FDirIcon);
       ...
       begin
        ...
        if FLoadIcons then
         begin
          ...
          else if DirectoryExists(SPath) then ListItem.ImageIndex:=FI;
         end;
  ...
  FPath:
   begin
    if not DirectoryExists(ImPath) then
     begin
      if MessageBox(Application.Handle, PChar(LangText(107, 'Выбранный путь не существует! Продолжить выполнение команды?')), PChar(LangText(41, 'Внимание')), MB_ICONEXCLAMATION or MB_YESNO) <> ID_YES then Exit;
     end;
    RunCommand(ImPath, '');
   end;
  FFile:
...
procedure TFormExec.FormKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 case Key of
  VK_ESCAPE: Close;
