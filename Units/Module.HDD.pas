...
uses
  ...
  System.Classes, Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs, CMW.OSInfo,
  CMW.Utils, CMW.ModuleStruct, WMI, Subs, Vcl.StdCtrls, Vcl.ComCtrls,
  System.Generics.Collections;

type
  TDriverNums = TList<integer>;
  ...
  public
    ...
    function GetDriverNum(CBIndex: Integer): Integer;
...
uses
  System.IniFiles, CMW.Main, WbemScripting_TLB, Winapi.ActiveX, Winapi.RichEdit;
{$R *.dfm}
...
begin
  ...
  if Desc <> '' then
    ...
    ShowMessage('Информация по атрибуту отсутствует.');
  Ini.Free;
end;
...
function THDDUnit.GetDriverNum(CBIndex: Integer): Integer;
begin
  Result := FDriverNums[CBIndex];
end;
...
constructor THDDUnit.Create;
begin
  inherited;
  FDriverNums := TDriverNums.Create;
end;

destructor THDDUnit.Destroy;
begin
  FDriverNums.Free;
  inherited;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        GroupID := G2;
        if SmartResult.AttrNum[i] in AttrInform then
          ImageIndex := 3
        else if SmartResult.AttrNum[i] in AttrWarning then
          ImageIndex := 1
        else if SmartResult.AttrNum[i] in AttrError then
          ImageIndex := 2
        else
          ImageIndex := 0;
      end;
...
begin
  case ID of
    0:
      Result := 'SMART не доступен';
    1:
      Result := 'Частота возникновения ошибок';
    2:
      Result := 'Значение средней производительности диска';
    3:
      Result := 'Время последнего разгона шпинделя';
    4:
      Result := 'Кол-во раз включения диска';
    5:
      Result := 'Кол-во секторов, переназначенных в резервную область';
    6:
      Result := 'Запас канала чтения (устар.)';
    7:
      Result := 'Частота возникновения ошибок при позиционировании БМГ';
    8:
      Result := 'Средняя производительность операций позиционирования головок';
    9:
      Result := 'Суммарное время работы';
    10:
      Result := 'Кол-во повторов запуска шпинделя';
    11:
      Result := 'Кол-во повторных попыток сброса накопителя';
    12:
      Result := 'Кол-во полных циклов «включение-отключение» диска';
    13:
      Result := 'Частота возникновения ошибок чтения (по вине ПО)';
    99:
      Result := 'Среднее FHC'; //
    100:
      Result := 'Температура гермоблока (Seagate) / Циклы стирания (SSD)';
    101:
      Result := 'Максимум FHC'; //
    103:
      Result := 'Восстановления таблицы перемещений'; //
  //108: Result:='Unknown ';//
    167:
      Result := 'SSD Режим защиты'; //
    168:
      Result := 'Кол-во ошибок SATA PHY';      //
    169:
      Result := 'Полное кол-во испорченных блоков'; //
    170:
      Result := 'Кол-во резервных блоков'; //
    171:
      Result := 'Кол-во ошибок программы'; //
    172:
      Result := 'Кол-во ошибок стирания'; //
    173:
      Result := 'Кол-во стираний'; //
    174:
      Result := 'Неожиданное отключение питания'; //
    175:
      Result := 'Кол-во испорченных таблиц кластерв'; //
    176:
      Result := 'Кол-во неудачных стираний'; //
    177:
      Result := 'Кол-во выравниваний износа'; //
    178:
      Result := 'Кол-во использующихся резервных блоков (SSD)'; //
    179:
      Result := 'Кол-во использующихся резервных блоков (SSD)'; //
    180:
      Result := 'Кол-во резервных секторов, доступных для ремапа';
    181:
      Result := 'Кол-во ошибок программы'; //
    182:
      Result := 'Кол-во ошибок стирания'; //
    183:
      Result := 'Кол-во неудачных попыток понижения режима SATA';
    184:
      Result := 'Кол-во ошибок, возникших при передаче данных через кэш';
    185:
      Result := 'Стабильность головок (WD)';
    186:
      Result := 'Обнаруженные индуцированные вибрации (Induced Op-Vibration Detection)'; //
    187:
      Result := 'Кол-во ошибок отбора секторов-кандидатов на переназначение';
    188:
      Result := 'Кол-во таймаут-операций';
    189:
      Result := 'Кол-во случаев записи при высоте полета головки выше рассчитанной';
    190:
      Result := 'Температура гермоблока (Hitachi, Samsung, WD)';
    191:
      Result := 'Кол-во критических ускорений';
    192:
      Result := 'Выключения питания в аварийных ситуациях / Кол-во парковок БМГ';
    193:
      Result := 'Кол-во полных циклов парковки/распарковки БМГ';
    194:
      Result := 'Текущая температура диска';
    195:
      Result := 'Кол-во ошибок, скорректированых аппаратными средствами ECC';
    196:
      Result := 'Кол-во операций переназначения секторов';
    197:
      Result := 'Кол-во секторов-кандидатов на переназначение в резервную область';
    198:
      Result := 'Кол-во секторов-кандидатов на переназначение (Самоконтроль)';
    199:
      Result := 'Кол-во ошибок, возникших при передаче в режиме UltraDMA';
    200:
      Result := 'Частота возникновения ошибок при записи';
    201:
      Result := 'Частота возникновения ошибок чтения (по вине ПО)';

    202:
      Result := 'Кол-во ошибок адресов данных (DAM)';
    203:
      Result := 'Кол-во ошибок ECC';
    204:
      Result := 'Кол-во ошибок ECC, скорректированных программным способом';
    205:
      Result := 'Кол-во ошибок, вызванных перегревом';
    206:
      Result := 'Высота между головкой и поверхностью диска';
    207:
      Result := 'Величина силы тока при раскрутке шпинделя';
    208:
      Result := 'Кол-во процедур вибрирующей раскрутки шпинделя';
    209:
      Result := 'Производительность поиска во время офлайновых операций';
    210:
      Result := 'Вибрация во время записи. (Maxtor 6B200M0 200GB и Maxtor 2R015H1 15GB)';
    211:
      Result := 'Вибрация во время записи';
    212:
      Result := 'Удары во время записи';
    220:
      Result := 'Сдвиг пластин диска относительно оси шпинделя';
    221:
      Result := 'Число ошибок, возникших из-за внешних нагрузок и ударов';
    222:
      Result := 'Время нахождения БМГ в перемещении';
    223:
      Result := 'Количество повторных циклов парковки/распарковки БМГ';
    224:
      Result := 'Величина силы трения БМГ при его выгрузке из парковочной области';
    225:
      Result := 'Количество циклов парковки БМГ';
    226:
      Result := 'Время выгрузки БМГ на диски';
    227:
      Result := 'Кол-во попыток скомпенсировать вращающий момент';
    228:
      Result := 'Кол-во повторов автоматической парковки БМГ, после отключения';
    229:
      Result := 'Спецификация производителя'; //
    230:
      Result := 'Амплитуда «дрожания»';
    231:
      Result := 'Температура диска / Остаток жизни (SSD)';
    232:
      Result := 'Кол-во завершенных циклов стирания от максимально возможного (%)';
    233:
      Result := 'Кол-во часов во вкл. состоянии / Media Wearout Indicator (SSD)';
    234:
      Result := 'Average erase count AND Maximum Erase Count'; //
    235:
      Result := 'Good Block Count AND System(Free) Block Count'; //
    240:
      Result := 'Время, затраченное на позиционирование БМГ (для SSD "Vendor Specific")';
    241:
      Result := 'Всего операций записи LBA (SSD)'; //
    242:
      Result := 'Всего операций чтения LBA (SSD)'; //
    249:
      Result := 'NAND_Writes_1GiB'; //
    250:
      Result := 'Кол-во ошибок во время чтения';
    254:
      Result := 'Кол-во падений диска (зафиксированное электроникой)';
  else
    Result := 'Неизвестный атрибут';
  end;
end;
...
procedure TFormHDD.FormKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  case Key of
    VK_ESCAPE:
      Close;
