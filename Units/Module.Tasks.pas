...
begin
  ...
  begin
    ...
    case TaskData.Version of
      tvOld:
        res := TasksUnit.FillValueList(ValueListEditor1, TasksUnit.TaskListXP[TaskData.TaskID]);
      tvNew:
        res := TasksUnit.FillValueList(ValueListEditor1, TasksUnit.TaskList[TaskData.TaskID]);
    else
      Exit;
    end;
    if res then
      ...
      tvOld:
        begin
          ...
          try
            ...
            if not FileExists(Info.WindowsPath + '\Tasks\' + TTaskData(LI.Data^).FilePath) then
              LI.Delete;
          except
        ...
        if Definition.Triggers.Count > 0 then
          ...
          begin
            case Definition.Triggers.Item[j].triggerType of
              ttEvent:
                Trigger := Trigger + LangText(72, 'по событию');
              ttTime:
                Trigger := Trigger + LangText(73, 'один раз');
              ttDaily:
                Trigger := Trigger + LangText(74, 'ежедневно');
              ttWeekly:
                Trigger := Trigger + LangText(75, 'еженедельно');
              ttMonthly:
                Trigger := Trigger + LangText(76, 'ежемесячно');
              ttMonthlyDOW:
                Trigger := Trigger + LangText(77, 'ежемесячно (в определенный день недели)');
              ttIdle:
                Trigger := Trigger + Trigger + LangText(78, 'во время бездействия');
              ttRegistration:
                Trigger := Trigger + LangText(79, 'при создании или изменении задачи');
              ttBoot:
                Trigger := Trigger + Trigger + LangText(80, 'при каждой загрузке');
              ttLogin:
                Trigger := Trigger + Trigger + LangText(81, 'при входе в систему');
              ttSessionStateChange:
                Trigger := Trigger + LangText(82, 'при блокировке/разблокировке раб. станции');
            end;
            if j <> Definition.Triggers.Count then
              Trigger := Trigger + ', ';
          end;
        if Trigger <> '' then
        begin
          ...
          Trigger := LangText(83, 'Отсутствуют');
          ...
          taCOMHandler:
            begin
              ...
              begin
                ...
                if Roll.OpenKey('LocalServer32', False) then
                  AddToValueEdit(VList, 'Local Server 32', Roll.ReadString(''), '');
                if Roll.OpenKey('InprocServer32', False) then
                  AddToValueEdit(VList, 'InProc Server 32', Roll.ReadString(''), '');
              end
              else
                tmp := comAction.ClassId;
...
begin
  if MessageBox(Application.Handle, 'Вы уверены, что хотите удалить задачу из списка?', 'Внимание', MB_ICONINFORMATION or MB_YESNO) <> ID_YES then
    Exit;
...
begin
  if MessageBox(Application.Handle, 'Вы уверены, что хотите отключить задачу?', 'Внимание', MB_ICONINFORMATION or MB_YESNO) <> ID_YES then
    Exit;
...
begin
  if MessageBox(Application.Handle, 'Вы уверены, что хотите включить задачу?', 'Внимание', MB_ICONINFORMATION or MB_YESNO) <> ID_YES then
    Exit;
  TTasksUnit(FLastUnit^).SetEnable(TListItem(FLastItem^), True);
end;

procedure TFormTask.FormKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  case Key of
    VK_ESCAPE:
      Close;
  ...
  begin
   ...
   //Добавление задач Task Scheduler 2.0
    ...
    begin
     ...
     {0}
          SubItems.Add(GetDateForTask(Task.LastRunTime));                        //Последний запуск
     {1}
          SubItems.Add(GetDateForTask(Task.NextRunTime));                        //След. запуск
     {2}
          SubItems.Add(TaskStateNames[Task.State]);                              //Состояние
     {3}
          SubItems.Add(Task.Path);                                               //Полный путь до файла
              ...
              taExec:
                begin
                  ...
                  begin
                    ...
                    begin
                      if GetFileIcon(IName, is16, TListView(ListView).SmallImages, II) > 0 then
                        ImageIndex := II;
                    end;
                  end;
                end;
  ...
  begin
   ...
   //Добавление задач Task Scheduler 1.0
    ...
    begin
     ...
     {0}
          SubItems.Add(GetDateForTask(TaskX.MostRecentRunTime));                 //Последний запуск
     {1}
          SubItems.Add(GetDateForTask(TaskX.NextRunTime));                       //След. запуск
     {2}
          SubItems.Add(TaskX.State);                                             //Состояние
     {3}
          SubItems.Add(TaskX.ApplicationName);                                   //Полный путь до файла
