...
const
  TCPState: array[0..12] of string = ('Неизвестно', 'Закрыт. Сокет не используется.',
...
begin
  ...
  begin
   ...
   Exists:Boolean}
    ...
    AddToValueEdit(ValueListEditor1, 'Тип соединения', ConTypeStr[dwType], 'Не известно');
    ...
    AddToValueEdit(ValueListEditor1, 'Локальный порт', Ini.ReadString(ConTypeStr[dwType], IntToStr(dwLocalPort), ''), '');
    AddToValueEdit(ValueListEditor1, 'Удалённый порт', Ini.ReadString(ConTypeStr[dwType], IntToStr(dwRemotePort), ''), '');
...
begin
  ...
  if Result = gsFinished then
    ...
    Inform(LangText(-1, 'Список активных портов не получен.'));
  OnChanged;
end;
...
begin
  ...
  try
   ...
   //Если не смог получить подробную инфу о подключениях, пробуем общую получить
      ...
      if not Step then
        Log(['Не смог получить любую информацию о TCP-подключениях']);
