    ...
    try
     ...
     with ListView.Items.Add do
      begin
       ...
       if Integer(objPolicy.FirewallEnabled) = VRAI then SubItems.Add('Включен') else SubItems.Add('Отключен');
       ImageIndex:=DI;
      end;
     with ListView.Items.Add do
      begin
       Caption:='Исключения разрешены';
       if Integer(objPolicy.ExceptionsNotAllowed) = VRAI then SubItems.Add('Нет') else SubItems.Add('Да');
       ImageIndex:=DI;
      end;
     with ListView.Items.Add do
      begin
       Caption:='Уведомления отключены';
       if Integer(objPolicy.NotificationsDisabled) = VRAI then SubItems.Add('Да') else SubItems.Add('Нет');
       ImageIndex:=DI;
      end;
     with ListView.Items.Add do
      begin
       Caption:='Unicast responses to multicast broadcast disabled';
       if Integer(objPolicy.UnicastResponsestoMulticastBroadcastDisabled) = VRAI then SubItems.Add('Да') else SubItems.Add('Нет');
       ImageIndex:=DI;
      end;
    except
     begin
