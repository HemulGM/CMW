...
function CheckYesNo(const value: string): boolean;
begin
  result := (LowerCaseAnsi(AnsiString(Copy(value, 1, 1))) = 'ä') or (LowerCaseAnsi(AnsiString(Copy(value, 1, 1))) = 'y') or (value = '1');  // 7 Aug 2010
end;
...
function CheckTrueFalse(const value: string): boolean;
begin
  result := (LowerCaseAnsi(AnsiString(Copy(value, 1, 1))) = 'è') or (value = '1'); // 7 Aug 2010
end;
...
function PerfCountGetMillStr(startLI: int64): string;
begin
  result := LInt2CEStr(PerfCountGetMilli(startLI)) + 'ìñ.';
end;
...
// These strings are NOT to be resourced
  ...
  Months: array[1..13] of string = ('ßíâ', 'Ôåâ', 'Ìàð', 'Àïð', 'Ìàé', 'Èþí', 'Èþë', 'Àâã', 'Ñåí', 'Îêò', 'Íîÿ', 'Äåê', '');
  DaysOfWeek: array[1..7] of string = ('Âñê', 'Ïíä', 'Âòð', 'Ñðä', '×òâ', 'Ïÿò', 'Ñóá');
...
begin
  ...
  if Bytes then  // 20 Oct 2011 improve result a little
    result := Trim(Format(mask, [float2])) + space + suffix + 'Áàéò'
  else
