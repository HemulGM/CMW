...
begin
  try
    ...
    begin
      Result := Result + 'Всего: ' + Format('%0.0f Мбайт', [dwTotalPhys div 1024 / 1024]) + ' | ' + 'Файл подкачки: ' + Format('%0.0f Мбайт', [dwTotalPageFile div 1024 / 1024]);
    end;
  except
    begin
...
begin
  try
    ...
    begin
      ...
      Result := 'Класс x' + IntToStr(lpSystemInfo.dwProcessorType);
    Roll.CloseKey;
    Roll.Destroy;
  except
    begin
      ...
      try
        case Roll.ReadInteger('AUOptions') of
          1:
            Result := 'Отключено';
          2:
            Result := 'Включено частично (Поиск обновлений)';
          3:
            Result := 'Включено частично (Поиск и загрузка обновлений)';
          4:
...
begin
  try
    ...
    SWbemLocator.Free;
    if ls = 0 then
      Result := 'Активация Windows выполнена'
    else
      Result := 'Активация Windows не выполнена';
  except
    begin
...
begin
  ...
  begin
    ...
    OSVersion := winOld;                      //Более ранние операционные системы (программа всё равно на них не будет работать)
end;
