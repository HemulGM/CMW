...
interface
 ...
 uses
  ...
  ShlObj, ValEdit, Vcl.StdCtrls, Vcl.ImgList,
  //
  OSInfo;
  //
...
implementation
 uses Registry, Main, Forms, ShellAPI, StructUnit, Vcl.FileCtrl, System.Win.ComObj, Winapi.ActiveX, Utils;
...
begin
 case Level of
  mlInfo: begin Cap:='Информация'; Icon:=MB_ICONINFORMATION; end;
  mlWarning: begin Cap:='Внимание'; Icon:=MB_ICONWARNING; end;
  mlError: begin Cap:='Ошибка'; Icon:=MB_ICONERROR; end;
 end;
...
begin
 ...
 for i:= 0 to List.Count - 1 do
  if tmp = AnsiLowerCase(List.Strings[i]) then Exit(False);
 try
  List.Add(Item);
  Result:=True;
 except
...
begin
 ...
 for i:= 1 to Count do LI.SubItems.Add('');
end;

function BoolStr(Value:Boolean; const VTrue, VFalse:string):string;
begin
 if Value then Exit(VTrue) else Exit(VFalse);
end;
...
begin
 ...
 if deltax > deltay then
  begin // horizontal or vertical
   if y2 > y1 then dydx:= -(deltay / deltax) else dydx:=deltay / deltax;
   if x2 < x1 then
    begin
  ...
  begin
   if x2 > x1 then dydx:= -(deltax / deltay) else dydx:= deltax / deltay;
   if y2 < y1 then
    begin
...
const faSpecial = faHidden or faSysFile or faDirectory;
begin
...
var Y, M, D:string;
begin
 if InstDate.Length < 8 then Exit(Def);
...
begin
 ...
 if Result then
  begin
   FExecuting:=True;
   FPID:=PI.dwProcessId;
   if not WaitStop(FPID) then Exit(False);
  ...
  begin
   ...
   if (SearchRec.Attr and faDirectory = faDirectory) then
    begin
     ...
     then
      begin
       if not ForceRemoveDir(sFileName) then Break;
       Result:=False;
      end;
    end
   else
    begin
     if not DeleteFile(sFileName) then Result:=False;
    end;
...
begin
 ...
 for i:= 0 to LV.Items.Count - 1 do
  begin
   if LV.Items[i].GroupID = GID then Inc(Result);
...
procedure ReplaceSysVar(var Src:string);
var FullPath:array [0..MAX_PATH - 1] of Char;
begin
 try
...
begin
 ...
 if not FileExists(Str) then
  begin
   ...
   if MessageBox(Application.Handle, PChar('Не смог обнаружить файл: '+Str+#13#10'Попробовать открыть каталог файла?'), 'Внимание', MB_YESNO or MB_ICONWARNING) <> ID_YES then Exit;
   repeat
...
begin
 ...
 else Log(['ILCreateFromPath(PChar(FileName)) = nil. FileName=', FileName]);
end;

//Получить метки ЖД по порядку в строке
function GetHDDrives:string;
const DRIVE_FIXED = 3;
var HResult:Cardinal;
...
begin
 ...
 if HResult > SizeOf(Buffer) then raise Exception.Create(SysErrorMessage(ERROR_OUTOFMEMORY));
 Drive:=Buffer;
 while Drive^ <> #0 do
  begin
   if GetDriveType(Drive) = DRIVE_FIXED then Result:=Result + Drive[0];
...
function LangText(ID:Integer; Text:string):string;
begin
 if LangH <= 0 then Result:=Text else Result:=LoadString(64000 + ID);
...
function DelFLSpace(str:string):string;
begin
 if str = '' then Exit(str);
 while str[1] = ' ' do str:=Copy(str, 2, Length(str) - 1);
 while str[Length(str)] = ' ' do str:=Copy(str, 1, Length(str) - 1);
...
var i:Word;
    NewGroup:TListGroup;
begin
 if GroupName = '' then GroupName:='Без группы';
 ...
 with NewGroup do
  begin
   ...
   else NewGroup.State:=[lgsNormal, lgsCollapsible];
  end;
end;
...
begin
 Result:=True;
 if UseEvtInf = 2 then Exit;
 if ListOfEventType.Count = 0 then Exit;
 for i:= 0 to ListOfEventType.Count - 1 do
  begin
   if Length(ListOfEventType.Strings[i]) <= 0 then Continue;
   if ListOfEventType.Strings[i][1] = '#' then Continue;
   if UseEvtInf = 0 then
    if ListOfEventType.Strings[i]='[EventSource]' then
     begin
      for j:= i + 1 to ListOfEventType.Count - 1 do
       begin
        if j > ListOfEventType.Count - 1 then Break;
        if Length(ListOfEventType.Strings[j]) <= 0 then Continue;
        if ListOfEventType.Strings[j][1] = '#' then Continue;
        UseEvtInf:=1;
        Break;
       end;
     end
    else Continue;
   if UseEvtInf = 0 then Break;
   if AnsiLowerCase(EventStr) = AnsiLowerCase(ListOfEventType.Strings[i]) then Exit;
  end;
 if UseEvtInf = 0 then
  begin
   UseEvtInf:=2;
...
begin
 for i:= Low(Value) to High(Value) do
  case Value[i].VType of
   vtInteger:   Result:=Result+IntToStr(Value[i].VInteger)+' ';
   vtString:    Result:=Result+AnsiString(Value[i].VPWideChar)+' ';
   vtBoolean:   Result:=Result+IntToStr(Ord(Value[i].VBoolean))+' ';
   vtChar:      Result:=Result+AnsiString(Value[i].VPWideChar)+' ';
   vtExtended:  Result:=Result+FloatToStr(Value[i].VExtended^)+' ';
   vtPointer:   Result:=Result+IntToStr(Integer(@Value[i].VPointer))+' ';
   vtPChar:     Result:=Result+AnsiString(Value[i].VPWideChar)+' ';
   vtObject:    if Value[i].VObject <> nil then
                 Result:=Result+Value[i].VObject.ClassName+' '
                else Result:=Result+'Object is nil ';
   vtClass:     if Value[i].VClass <> nil then
                 Result:=Result+Value[i].VClass.ClassName+' '
                else Result:=Result+'Class is nil ';
   vtWideChar:  Result:=Result+AnsiString(Value[i].VWideChar)+' ';
   vtPWideChar: Result:=Result+AnsiString(Value[i].VWideChar)+' ';
   vtAnsiString:Result:=Result+AnsiString(Value[i].VPWideChar)+' ';
   vtCurrency:  Result:=Result+FloatToStr(Currency(Value[i].VCurrency^))+' ';
   vtVariant:   Result:=Result+'@Variant@ ';
   vtInterface: Result:=Result+'@Interface@ ';
   vtWideString:Result:=Result+AnsiString(Value[i].VPWideChar)+' ';
   vtInt64:     Result:=Result+IntToStr(Integer(Value[i].VInt64^))+' ';
   vtUnicodeString:Result:=Result+AnsiString(Value[i].VPWideChar)+' ';
  end;
 LogBuf:=Result;
 AddToLogList(LogBuf);
 if NotUseLog then Exit;
 Logging;
...
procedure Unload;
begin
 if (SLog = nil) or (not Assigned(SLog)) then Exit;
 if (LogList = nil) or (not Assigned(LogList)) then Exit;
 if SLog.Count > 0 then
  begin
   LogList^.Lines.AddStrings(SLog);
   if (SLog <> nil) then SLog.Clear;
   FreeAndNil(SLog);
  end;
end;
...
procedure AddToLogList(Text:string);
begin
 if (LogList = nil) or (not Assigned(LogList)) then
  begin
   try
    if not Assigned(SLog) then SLog:=TStringList.Create;
...
begin
 ...
 for i:=1 to TaskCollection.Count do TL.Add(TaskCollection.Item[i]);
 if AllFolder then
  begin
   TaskFolder:=Folder.GetFolders(0);
   for i:=1 to TaskFolder.Count do GetTasks(TaskFolder.Item[i], AllFolder, TL);
  end;
end;
...
begin
 ...
 if TaskSched.TaskCount > 0 then
  for i:=0 to TaskSched.TaskCount - 1 do TL.Add(TaskSched.Tasks[i]);
end;
...
begin
 ...
 if (RK = 'HKCR') or (RK = 'HKEY_CLASSES_ROOT') then Exit(HKEY_CLASSES_ROOT);
 if (RK = 'HKCU') or (RK = 'HKEY_CURRENT_USER') then Exit(HKEY_CURRENT_USER);
 if (RK = 'HKLM') or (RK = 'HKEY_LOCAL_MACHINE') then Exit(HKEY_LOCAL_MACHINE);
 if (RK = 'HKUR') or (RK = 'HKEY_USERS') then Exit(HKEY_USERS);
 if (RK = 'HKPD') or (RK = 'HKEY_PERFORMANCE_DATA') then Exit(HKEY_PERFORMANCE_DATA);
 if (RK = 'HKCC') or (RK = 'HKEY_CURRENT_CONFIG') then Exit(HKEY_CURRENT_CONFIG);
 if (RK = 'HKDD') or (RK = 'HKEY_DYN_DATA') then Exit(HKEY_DYN_DATA);
end;

//Исключение для дат задач
function GetDateForTask(Value:TDate):string;
begin
 if Value = 0 then Result:=LangText(69, 'Никогда') else Result:=DateTimeToStr(Value);
end;
...
begin
 ...
 case ParamSort of
  ...
  begin
   try
    if AnsiLowerCase(item1.Caption) > AnsiLowerCase(item2.Caption) then Result:= 1 else
    if AnsiLowerCase(item1.Caption) < AnsiLowerCase(item2.Caption) then Result:= -1;
   except
    Exit;
   end;
  end;
  ...
  try
   if AnsiLowerCase(item1.SubItems[ParamSort]) > AnsiLowerCase(item2.SubItems[ParamSort]) then Result:= 1 else
   if AnsiLowerCase(item1.SubItems[ParamSort]) < AnsiLowerCase(item2.SubItems[ParamSort]) then Result:= -1;
  except
   Exit;
  end;
 end;
...
begin
 ...
 case ParamSort of
  ...
  begin
   try
    if not TryStrToInt(Item1.Caption, I1) then Exit;
    if not TryStrToInt(Item2.Caption, I2) then Exit;
   except
    Exit;
   end;
  end;
  ...
  try
   if not TryStrToInt(Item1.SubItems[ParamSort], I1) then Exit;
   if not TryStrToInt(Item2.SubItems[ParamSort], I2) then Exit;
  except
   Exit;
  end;
 end;
...
begin
 ...
 while Seconds > GetTickCount do Application.ProcessMessages;
end;
   ...
   repeat
    ...
    if (SearchRec.Name <> '..') and (SearchRec.Name <> '.') then
     begin     {
      ...
      if (SearchRec.Attr and faSysFile) = faSysFile then Continue;
      if (SearchRec.Attr and faDirectory) = faDirectory then
...
begin
 ...
 if InsMinus then Result:='-'+Result;
end;
...
begin
 ...
 Result:=FileN;
 if FileExists(path0+FileN) then Exit(path0+FileN);
 if FileExists(path1+FileN) then Exit(path1+FileN);
 if FileExists(path2+FileN) then Exit(path2+FileN);
 if FileExists(path3+FileN) then Exit(path3+FileN);
 if FileExists(path4+FileN) then Exit(path4+FileN);
 if FileExists(path5+FileN) then Exit(path5+FileN);
 if FileExists(path6+FileN) then Exit(path6+FileN);
 if FileExists(path7+FileN) then Exit(path7+FileN);
end;
...
begin
 ...
 while Pos(#13, FN) <> 0 do Delete(FN, Pos(#13, FN), 1);
 while Pos(#10, FN) <> 0 do Delete(FN, Pos(#10, FN), 1);
 while Pos(#0, FN) <> 0 do Delete(FN, Pos(#0, FN), 1);
 if (FN[1] = '"') then
  begin
 ...
 if FN.Length < 3 then Exit;

 if FN[1] = '@' then FN:=Copy(FN, 2, Length(FN)-1);

 RepVar(FN, '/', '\');
 if ExtractFilePath(FN) = '' then FN:=SetSysPath(FN);
 if FN.Length < 3 then Exit;
 if (FN[1] = '\') or (FN[1] = '/') then Delete(FN, 1, 1);
